...
class SingleAgentAccessMgr(AgentAccessMgr):
    ...
    def _create_policy(self) -> NetPolicy:
        ...
        if self._config.habitat_baselines.rl.ddppo.pretrained and (self.agent_name == "agent_0" or self.agent_name == "main_agent") : 
            # Handle multiple possible prefixes in pretrained models
            possible_prefixes = ["actor_critic.", "agent_0_actor_critic.", "agent_0_", "net.", "agent_0_net.", ""]
            ...
            for prefix in possible_prefixes:
                matches = sum(1 for k in pretrained_state["state_dict"].keys() 
                             if k.startswith(prefix) and k[len(prefix):] in model_state_dict)
                if matches > max_matches:
                    max_matches = matches
                    best_prefix = prefix
            
            # Load with the best matching prefix and shape checking
            ...
            for k, v in pretrained_state["state_dict"].items():
                if k.startswith(best_prefix):
                    ...
                    if new_key in model_state_dict and v.shape == model_state_dict[new_key].shape:
                        filtered_pretrained_state_dict[new_key] = v
                    else:
                        print(f"Skipping {k} -> {new_key}: shape mismatch or key not found")

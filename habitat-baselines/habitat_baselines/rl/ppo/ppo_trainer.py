...
class PPOTrainer(BaseRLTrainer):
    ...
    def _init_train(self, resume_state=None):
        ...
        if self._is_static_encoder:
            ...
            if self._encoder is  None:
                self._encoder = self._agents[0].actor_critic.visual_encoder
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        # # 假设你的 trainer 类中有 self.num_steps_done 这个变量
        # save_path = f"rollouts/rollouts_data_step_{self.num_steps_done}.pt"
        # torch.save(self._agent.rollouts, save_path)
        # print(f"Rollouts 数据已保存到: {save_path}")
    ...
    def _save_rollout_video(self):
        """
        一个辅助函数，用于将当前完成的 rollout 保存为视频文件。
        它会自动检测 RGB 或 Depth 传感器。
        """
        import imageio
        import os

        # 确保视频保存的文件夹存在
        output_folder = "rollouts"
        os.makedirs(output_folder, exist_ok=True)

        # 从 agent 的 rollouts 存储中获取数据
        rollouts = self._agent.rollouts._active_storages[0]
        if rollouts is None:
            return

        # --- 智能检测传感器 ---
        obs_keys = rollouts.buffers.get('observations', {}).keys()
        # 优先使用 RGB 传感器
        rgb_sensor_key = next((k for k in obs_keys if 'rgb' in k.lower()), None)
        # 如果没有 RGB，则使用您指定的 Depth 传感器
        depth_sensor_key = 'articulated_agent_jaw_depth'

        sensor_to_use = None
        is_depth = False
        if rgb_sensor_key is not None:
            sensor_to_use = rgb_sensor_key
            logger.info(f"找到 RGB 传感器 '{sensor_to_use}' 用于生成视频。")
        elif depth_sensor_key in obs_keys:
            sensor_to_use = depth_sensor_key
            is_depth = True
            logger.info(f"未找到 RGB 传感器，将使用 Depth 传感器 '{sensor_to_use}'。")
        else:
            # 如果两个都找不到，就不生成视频
            return

        # --- 提取并处理帧 ---
        # 我们只可视化第一个环境 (env_index = 0)
        observations = rollouts.buffers['observations'][sensor_to_use][:, 0]
        observations = observations.cpu().numpy()

        frames = []
        if is_depth:
            # 如果是深度图，需要归一化才能看到
            max_depth = np.max(observations[np.isfinite(observations)])
            if max_depth == 0: max_depth = 1.0 # 防止除以零
            
            for depth_frame in observations:
            ...
            # 如果是 RGB 图，直接使用
            frames = [frame.astype(np.uint8) for frame in observations]
        
        # --- 保存视频 ---
        # 使用 self.num_steps_done 来确保视频文件名与训练进度同步
        video_filename = f"rollout_video_step_{self.num_updates_done}.mp4"
        output_path = os.path.join(output_folder, video_filename)
        
        try:
            imageio.mimsave(output_path, frames, fps=10)
            logger.info(f"Rollout 视频已成功保存到: {output_path}")

...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        # Determine which visual observations are present (excluding RGB)
        self.visual_keys = [
            k
            for k, v in observation_space.spaces.items()
            if len(v.shape) > 1 and k != ImageGoalSensor.cls_uuid and k != "oracle_humanoid_future_trajectory" and "rgb" not in k.lower()
        ]
...
class ResNetCLIPEncoder(nn.Module):
    def __init__(
        ...
        self.visual_keys = [
            k
            for k, v in observation_space.spaces.items()
            if len(v.shape) > 1 and k != ImageGoalSensor.cls_uuid and "rgb" not in k.lower()
        ]
